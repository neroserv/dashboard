...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Neuer Plan', href: '#' },
];

const isActive = ref(true);
const hostingServerId = ref(
    props.pterodactylHostingServers[0]?.id ? String(props.pterodactylHostingServers[0].id) : '',
);
const sortOrder = ref(0);
const configMaxCpu = ref('');
const configMaxMemoryMb = ref('');
const configMaxDiskGb = ref('');
const configNestId = ref('');
const configEggId = ref('');

const configPlanOptions = ref<PlanOption[]>([]);
...
<template>
    <AdminLayout :breadcrumbs="breadcrumbs">
        <Head title="Gameserver-Cloud-Plan anlegen" />

        <div class="space-y-6">
            <div>
                <Heading level="h1">Neuer Gameserver-Cloud-Plan</Heading>
            ...
            <Form
                action="/admin/gameserver-cloud-plans"
                method="post"
                class="block"
                v-slot="{ errors }"
            >
